// Team Leader dashboard — same data access as admin, but no delete.
...
import {
  ...
  IconEdit, IconExternal, IconSort, IconBatch, IconFilter,
} from '../../lib/icons.jsx';
  ...
  return (
    <div className="page">
      <div className="page-head">
        <div>
          <h1>Dashboard</h1>
          <p className="muted">All content across your team.</p>
        </div>
      ...
      <div className="stats-row">
        <StatCard tone="blue" label="Active" value={periodStats.active} hint="not yet completed" />
      ...
      <div className="filter-bar">
        <div className="search">
          <IconSearch />
          <input value={filters.q} onChange={(e) => setFilters((f) => ({ ...f, q: e.target.value }))} placeholder="Search by title…" />
        </div>
        ...
        {filterActive && (
          <button type="button" className="link-btn" onClick={() => setFilters({ q: '', status: '', writer: '', client: '' })}>Clear</button>
        )}
      </div>

      <div className="card no-pad">
        {filtered.length === 0 ? (
          <EmptyState title="No content matches" message="Try adjusting your filters." />
        ) : (
          <>
                ...
                <tr>
                  {SORTABLE.map((col) => (
                    <th key={col.key} className="sortable" onClick={() => toggleSort(col.key)}>
                      {col.label}<IconSort dir={sort.key === col.key ? sort.dir : null} />
                    </th>
                  ))}
                  <th>Type</th><th>Links</th><th>TTW</th><th aria-label="Actions" />
                </tr>
              </thead>
              <tbody>
                {pageRows.map((a) => {
                  ...
                  return (
                    <tr key={a.id} className={`row-link ${overdue ? 'row-overdue' : ''}`} onClick={() => navigate(`/tl/articles/${a.id}`)}>
